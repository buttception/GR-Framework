...
EnemyTuck::EnemyTuck(std::string _meshName, Vector3 position) : EnemyEntity(_meshName)
{
...
void EnemyTuck::Update(double dt)
{
	if (!active)
		return;
...
void EnemyTuck::CollisionResponse(GenericEntity * thatEntity)
{
	if (active) {
...
void EnemyTuck::Attack(GenericEntity * thatEntity, double dt)
{
...
EnemyTuck * Create::Tuck(std::string _meshName, Vector3 position)
{
	EnemyTuck* tuck = new EnemyTuck(_meshName, position);
	EntityManager::GetInstance()->AddEntity(tuck);
	return tuck;
}
