...
SceneLose*SceneLose::sInstance = new SceneLose(SceneManager::GetInstance());
...
SceneLose::SceneLose(SceneManager*_sceneMgr)
{
	_sceneMgr->AddScene("Lose", this);
}
...
void SceneLose::Init()
{
	...
	//Main Menu
	MeshBuilder::GetInstance()->GenerateQuad("Scene_Lose", Color(1, 1, 1), 1.f);
	MeshList::GetInstance()->GetMesh("Scene_Lose")->textureID[0] = LoadTGA("Image//Menu/Lose.tga");
	...
	MainMenu = Create::Sprite2DObject("Scene_Lose",
...
void SceneLose::Update(double dt)
{
	...
	if (b_Return)
		Arrow->SetPosition(Vector3((float)Application::GetInstance().GetWindowWidth() / 2.85f,
