...
*/
bool MeshBuilder::checkMesh(std::string meshName)
{
	if (MeshList::GetInstance()->GetMesh(meshName))
		return true;
	else
		return false;
}
...
Mesh* MeshBuilder::GenerateAxes(const std::string &meshName, float lengthX, float lengthY, float lengthZ)
{
	...
	return MeshList::GetInstance()->GetMesh(meshName);
}
...
Mesh * MeshBuilder::GenerateGround(const std::string & meshName, Color color, float length)
{
	if (!checkMesh(meshName)) {
		Vertex v;
		std::vector<Vertex> vertex_buffer_data;
		...
		v.color = color;
		v.normal.Set(0, 0, 1);
		v.texCoord.Set(5.0f, 0);
		vertex_buffer_data.push_back(v);
		v.pos.Set(0.5f * length, 0.5f * length, 0);
		v.color = color;
		v.normal.Set(0, 0, 1);
		v.texCoord.Set(5.0f, 5.0f);
		vertex_buffer_data.push_back(v);
		v.pos.Set(-0.5f * length, 0.5f * length, 0);
		v.color = color;
		v.normal.Set(0, 0, 1);
		v.texCoord.Set(0, 5.0f);
		vertex_buffer_data.push_back(v);
